...
// As catro propostas de marca, á espera de que se escolla unha.
// O set «Orixinal» borrouse enteiro: non gustaba e ninguén o usaba.
// Os seus 16 «extras» (buscar, editar, eliminar…) tampouco estaban
// chamados desde ningures; cando faga falta un, pídese no estilo que
// estea activo, non se recupera o vello.
const MARCA = {
  logo1: (
...
// ⚠️ `estilo2` é ademais o FALLBACK: cando un estilo incompleto non ten
// un icono, tírase de aquí. Por iso ten que estar sempre completo.
const RESERVA = { ...ALTERNATIVOS.estilo2, ...MARCA };
const SETS = { estilo2: ALTERNATIVOS.estilo2, emojis: EMOJIS, ...ALTERNATIVOS };
...
export const ESTILOS = Object.keys(SETS).map((id) => ({
  id,
  nome: id === 'estilo2' ? 'Xeométrico minimal'
    : id === 'emojis' ? 'Emojis' : (NOMES_ESTILO[id] || id),
  ...cobertura(id),
}));
...
export const NOMES_ICONA = Object.keys(RESERVA);

export function hayIcona(nome) {
  return Object.prototype.hasOwnProperty.call(RESERVA, nome);
}
...
export function Icona({ nome, size = 24, cor, style, title, estilo }) {
  ...
  const set = SETS[activo] || RESERVA;
  // Un estilo incompleto non pode deixar ocos: cae á reserva.
  const formas = set[nome] || RESERVA[nome];
